...
{
	$values = array($type); #$accepted, $_GET['class'], $_GET['wztype'], $char, $faction);
	...
	$sql = rtrim($sql, ',');
	$sql .= ") and wztype like ? and class = ? and `character` like ? and faction like ? ";
	$sql .= "and patch in (";
	...
	array_push($values, $_GET['class'], $char, $faction);
